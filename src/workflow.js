 ...
 */
import { createMessageWithMetadata } from "./metadata.js";
 ...
 */
function logPruningResults(withMetadata, finalCount, config) {
    const logger = getLogger();
    ...
    logger.info(`Filter phase complete: ${prunedCount} pruned, ${finalCount} kept (${originalCount} total)`);
    if (config.debug && prunedCount > 0) {
